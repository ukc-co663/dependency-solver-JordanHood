...
Installer.buildInstallCommands = (stateObject) => {
  const reducedCommands = _.reduce(stateObject.resolvedPackages, (commands, packageToInstall) => {
    // ["+I=9","+G=7","+E=8","+D=0","+C=3","+B=2","+A=1"] is the aim for seen 2
    // currently got a dup on ["+I=9","+G=7","+E=8","+D=0","+C=3","+I=9","+G=7","+E=8","+D=0","+C=3","+B=2","+A=1"]
   _.forEachRight(packageToInstall.allDeps, dep => commands.push(`${(packageToInstall.constraintData.install) ? '+' : '-'}${dep}`))
